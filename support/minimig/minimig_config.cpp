...
const char *config_cpu_msg[] = { "68000", "------", "68Arm","68020"};
...
static void ApplyConfiguration(char reloadkickstart)
{
	...
	printf("Fast RAM size : %s\n", config_memory_fast_msg[minimig_020plus(minimig_config.cpu)][((memcfg >> 4) & 0x03) | ((memcfg & 0x80) >> 5)]);
...
int minimig_cfg_load(int num)
{
	...
	if ((minimig_config.cpu&3) ==1 ) minimig_config.cpu = 0;
	...
	sprintf(cfg_str, "CPU: %s, Chipset: %s, ChipRAM: %s, FastRAM: %s, SlowRAM: %s",
		...
		config_memory_chip_msg[(minimig_config.memory >> 0) & 0x03], config_memory_fast_msg[minimig_020plus(minimig_config.cpu)][((minimig_config.memory >> 4) & 0x03) | ((minimig_config.memory & 0x80) >> 5)], config_memory_slow_msg[(minimig_config.memory >> 2) & 0x03]
	);
	BootPrintEx(cfg_str);
